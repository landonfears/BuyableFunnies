...
BuyableFunnies.prototype = {
	...
	init : function(){
		var script_info = this.getParams(this.script);
		this.path = decodeURIComponent(script_info['purl'])+this.path;
		...
		jQuery('.buyablefunnies_widget').waitForImages(function() {
			...
			BuyableFunniesWidget.key = script_info['key'];
	...
	showLoading : function(set_busy){
		jQuery('.buyablefunnies_widget_content_wrap').css('background-image', 'url('+this.path+'/images/bf-loader.gif)');	
...
function bfReady(){
	if(jQuery('.buyablefunnies_widget').length){
		// Only keep the first instance if there are multiple
		jQuery('.buyablefunnies_widget:not(:first)').remove();
